...
class UserForm(forms.ModelForm):
    ...
    def save(self, *args, **kw):  # noqa: PLR0912
        super().save(*args, **kw)
        evaluations_current_semester = self.instance.evaluations_participating_in.exclude(
            course__semester=Semester.active_semester()
        )
        new_evaluations = set(evaluations_current_semester) | set(self.cleaned_data.get("evaluations_participating_in"))
        old_evaluations = set(self.instance.evaluations_participating_in.all())

        self.instance.evaluations_participating_in.set(
            list(evaluations_current_semester) + list(self.cleaned_data.get("evaluations_participating_in"))
        )
